...
import com.onlyforchris.store.wx.vo.CouponVo;
...
public class WxCouponController {
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        if ((total != 0) && (totalCoupons >= total)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_EXCEED_LIMIT, "优惠券已领完");
        }
        ...
        if ((limit != 0) && (userCounpons >= limit)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_EXCEED_LIMIT, "优惠券已经领取过");
        }
        ...
        if (type.equals(CouponConstant.TYPE_REGISTER)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "新用户优惠券自动发送");
        } else if (type.equals(CouponConstant.TYPE_CODE)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券只能兑换");
        } else if (!type.equals(CouponConstant.TYPE_COMMON)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券类型不支持");
        }
        ...
        if (status.equals(CouponConstant.STATUS_OUT)) {
            ...
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券已经过期");
        }
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        if (coupon == null) {
            return ResponseUtil.failed(WxResponseCode.COUPON_CODE_INVALID, "优惠券不正确");
        }
        ...
        Integer totalCoupons = couponUserService.countCoupon(couponId);
        if ((total != 0) && (totalCoupons >= total)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_EXCEED_LIMIT, "优惠券已兑换");
        }
        ...
        if (type.equals(CouponConstant.TYPE_REGISTER)) {
            ...
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券只能领取，不能兑换");
        } else if (!type.equals(CouponConstant.TYPE_CODE)) {
            return ResponseUtil.failed(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券类型不支持");
        }
