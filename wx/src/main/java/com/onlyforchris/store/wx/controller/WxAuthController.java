...
import com.onlyforchris.store.dal.service.CouponAssignService;
...
import com.onlyforchris.store.wx.dto.WxLoginInfo;
import com.onlyforchris.store.wx.service.CaptchaCodeManager;
import com.onlyforchris.store.wx.service.UserTokenManager;
...
public class WxAuthController {
    ...
    public Object login(@RequestBody String body, HttpServletRequest request) {
        ...
        if (userList.size() > 1) {
            ...
            return ResponseUtil.failed(AUTH_INVALID_ACCOUNT, "账号不存在");
        } else {
            user = userList.get(0);
        }
        ...
        if (!encoder.matches(password, user.getPassword())) {
            return ResponseUtil.failed(AUTH_INVALID_ACCOUNT, "账号密码不对");
        }
    ...
    public Object registerCaptcha(@RequestBody String body) {
        ...
        if (!notifyService.isSmsEnable()) {
            return ResponseUtil.failed(AUTH_CAPTCHA_UNSUPPORT, "小程序后台验证码服务不支持");
        }
        String code = CharUtil.getRandomNum(6);
        boolean successful = CaptchaCodeManager.addToCache(phoneNumber, code);
        if (!successful) {
            return ResponseUtil.failed(AUTH_CAPTCHA_FREQUENCY, "验证码未超时1分钟，不能发送");
        }
    ...
    public Object register(@RequestBody String body, HttpServletRequest request) {
        ...
        List<User> userList = userService.queryByUsername(username);
        if (userList.size() > 0) {
            return ResponseUtil.failed(AUTH_NAME_REGISTERED, "用户名已注册");
        }

        userList = userService.queryByMobile(mobile);
        if (userList.size() > 0) {
            return ResponseUtil.failed(AUTH_MOBILE_REGISTERED, "手机号已注册");
        }
        if (!RegexUtil.isMobileSimple(mobile)) {
            return ResponseUtil.failed(AUTH_INVALID_MOBILE, "手机号格式不正确");
        }
        //判断验证码是否正确
        String cacheCode = CaptchaCodeManager.getCachedCaptcha(mobile);
        if (cacheCode == null || cacheCode.isEmpty() || !cacheCode.equals(code)) {
            return ResponseUtil.failed(AUTH_CAPTCHA_UNMATCH, "验证码错误");
        }
        ...
        if (!StringUtils.isEmpty(wxCode)) {
            try {
                ...
                e.printStackTrace();
                return ResponseUtil.failed(AUTH_OPENID_UNACCESS, "openid 获取失败");
            }
            ...
            if (userList.size() == 1) {
                ...
                if (!checkUsername.equals(openId) || !checkPassword.equals(openId)) {
                    return ResponseUtil.failed(AUTH_OPENID_BINDED, "openid已绑定账号");
                }
            }
        }
    ...
    public Object reset(@RequestBody String body, HttpServletRequest request) {
        ...
        if (userList.size() > 1) {
            ...
            return ResponseUtil.failed(AUTH_MOBILE_UNREGISTERED, "手机号未注册");
        } else {
            user = userList.get(0);
        }
