...
import com.onlyforchris.store.core.utils.IpUtil;
...
import com.onlyforchris.store.dal.constant.OrderHandleOption;
...
import com.onlyforchris.store.wx.task.OrderUnpaidTask;
...
import static com.onlyforchris.store.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            ...
            if (rules.getStatus().equals(GrouponConstant.RULE_STATUS_DOWN_EXPIRE)) {
                return ResponseUtil.failed(GROUPON_EXPIRED, "团购已过期!");
            }
            //团购规则已经下线
            if (rules.getStatus().equals(GrouponConstant.RULE_STATUS_DOWN_ADMIN)) {
                return ResponseUtil.failed(GROUPON_OFFLINE, "团购已下线!");
            }

            if (grouponLinkId != null && grouponLinkId > 0) {
                //团购人数已满
                if (grouponService.countGroupon(grouponLinkId) >= (rules.getDiscountMember() - 1)) {
                    return ResponseUtil.failed(GROUPON_FULL, "团购活动人数已满!");
                }
                // NOTE
                ...
                if (grouponService.hasJoin(userId, grouponLinkId)) {
                    return ResponseUtil.failed(GROUPON_JOIN, "团购活动已经参加!");
                }
                // （2）不允许参加自己开团的团购
    ...
    public Object cancel(Integer userId, String body) {
        ...
        if (!handleOption.isCancel()) {
            return ResponseUtil.failed(ORDER_INVALID_OPERATION, "订单不能取消");
        }
    ...
    public Object prepay(Integer userId, String body, HttpServletRequest request) {
        ...
        if (!handleOption.isPay()) {
            return ResponseUtil.failed(ORDER_INVALID_OPERATION, "订单不能支付");
        }

        User user = userService.findById(userId);
        String openid = user.getWeixinOpenid();
        if (openid == null) {
            return ResponseUtil.failed(AUTH_OPENID_UNACCESS, "订单不能支付");
        }
        ...
        try {
            ...
            e.printStackTrace();
            return ResponseUtil.failed(ORDER_PAY_FAIL, "订单不能支付");
        }
    ...
    public Object confirm(Integer userId, String body) {
        ...
        if (!handleOption.isConfirm()) {
            return ResponseUtil.failed(ORDER_INVALID_OPERATION, "订单不能确认收货");
        }
    ...
    public Object delete(Integer userId, String body) {
        ...
        if (!handleOption.isDelete()) {
            return ResponseUtil.failed(ORDER_INVALID_OPERATION, "订单不能删除");
        }
    ...
    public Object goods(Integer userId, Integer ogid) {
        ...
        OrderGoods orderGoods = orderGoodsService.findById(ogid);
        ...
        return ResponseUtil.success(orderGoods);
    }
    ...
    public Object comment(Integer userId, String body) {
        ...
        if (!OrderUtil.isConfirmStatus(order) && !OrderUtil.isAutoConfirmStatus(order)) {
            return ResponseUtil.failed(ORDER_INVALID_OPERATION, "当前商品不能评价");
        }
        if (!order.getUserId().equals(userId)) {
            return ResponseUtil.failed(ORDER_INVALID, "当前商品不属于用户");
        }
        Integer commentId = orderGoods.getComment();
        if (commentId == -1) {
            return ResponseUtil.failed(ORDER_COMMENT_EXPIRED, "当前商品评价时间已经过期");
        }
        if (commentId != 0) {
            return ResponseUtil.failed(ORDER_COMMENTED, "订单商品已评价");
        }
    ...
    public void releaseCoupon(Integer orderId) {
        List<CouponUser> couponUsers = couponUserService.findByOid(orderId);
