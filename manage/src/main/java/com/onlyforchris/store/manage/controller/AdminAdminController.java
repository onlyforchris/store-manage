...
public class AdminAdminController {
    ...
    private Object validate(Admin admin) {
        ...
        if (!RegexUtil.isUsername(name)) {
            return ResponseUtil.failed(ADMIN_INVALID_NAME, "管理员名称不符合规定");
        }
        String password = admin.getPassword();
        if (StringUtils.isEmpty(password) || password.length() < 6) {
            return ResponseUtil.failed(ADMIN_INVALID_PASSWORD, "管理员密码长度不能小于6");
        }
        return null;
    }
    ...
    public Object create(@RequestBody Admin admin) {
        ...
        if (adminList.size() > 0) {
            return ResponseUtil.failed(ADMIN_NAME_EXIST, "管理员已经存在");
        }
    ...
    public Object delete(@RequestBody Admin admin) {
        ...
        if (currentAdmin.getId().equals(anotherAdminId)) {
            return ResponseUtil.failed(ADMIN_DELETE_NOT_ALLOWED, "管理员不能删除自己账号");
        }
